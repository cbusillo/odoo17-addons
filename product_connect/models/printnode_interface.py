    ...
    def generate_label(
        ...
        if bottom_text:
            ...
            for line in bottom_text:
                label.add_default_font(
                    font_name=0, character_height=self.LABEL_TEXT_SIZE["small"], character_width=self.LABEL_TEXT_SIZE["small"]
                )
        ...
        return base64.b64encode(zpl_text_with_quantity.encode("utf-8")).decode("utf-8")

    @staticmethod
    def combine_labels(labels: list[str]) -> str:
        decoded_labels = [base64.b64decode(label).decode("utf-8") for label in labels]
        combined_labels = "".join(decoded_labels)
        combined_labels_base64 = base64.b64encode(combined_labels.encode("utf-8")).decode("utf-8")
